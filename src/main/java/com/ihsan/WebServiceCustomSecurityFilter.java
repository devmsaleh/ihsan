...
import com.ihsan.service.UserTokenService;
...
public class WebServiceCustomSecurityFilter extends GenericFilterBean {
    ...
    List<String> publicUrls = Arrays.asList("/login", "/swagger.json", "/getCouponImage", "getNewProjectTypeImage", "getNewProjectCountryImage", "getSponsorshipCountryImage", "getOrphanImage", "refreshApplicationCache");
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        ...
        if (isAllowedUrl(request.getRequestURI())) {
            // logger.info("######### WebServiceCustomSecurityFilter > login service > chain.doFilter");
            chain.doFilter(servletRequest, servletResponse);
        } else {
            // logger.info("######### WebServiceCustomSecurityFilter > token check,token: " + request.getHeader("token"));
